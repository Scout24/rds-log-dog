...
BASE_SCHEDULER_STACK_NAME=${SCHEDULER_STACK_NAME}
...
cp ${BASE_SCHEDULER_STACK_NAME}.yaml ${SCHEDULER_STACK_NAME}.yaml
sed -i "s/^\(\s*\)${BASE_SCHEDULER_STACK_NAME}:/\1${SCHEDULER_STACK_NAME}:/" ${SCHEDULER_STACK_NAME}.yaml
sed -i "s/\(\|ref\|\)${BASE_FUNCTION_STACK_NAME}\./\1${FUNCTION_STACK_NAME}./" ${SCHEDULER_STACK_NAME}.yaml
...
grep -q "${SCHEDULER_STACK_NAME}.yaml" ".gitignore" || echo "${SCHEDULER_STACK_NAME}.yaml" >> .gitignore

echo "created new stack definitions from orginal ones:"
echo "${DST_BUCKET_STACK_NAME}.yaml"
echo "${FUNCTION_STACK_NAME}.yaml"
echo "${SCHEDULER_STACK_NAME}.yaml"
